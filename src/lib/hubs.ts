...
export function getTrailsByCultureValleyId(cultureValleyId: string): Trail[] {
  // Raggruppa SOLO per la valle reale del sentiero (assegnata per coordinate).
  // I `municipalities` nei dati del Catasto sono inaffidabili e facevano
  // "sconfinare" sentieri nella valle sbagliata (es. sentieri di Cogne nel Lys).
  return getAllTrails()
